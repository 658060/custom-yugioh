--Leaf-shaped Behe-moth
local s, id = GetID()
function s.initial_effect(c)
	-- discard; banish from deck
	...
	c:RegisterEffect(e1)
	-- if banished, banish insect and bounce card to return to gy
	local e2 = Effect.CreateEffect(c)
	e2:SetCategory(CATEGORY_TOGRAVE + CATEGORY_REMOVE + CATEGORY_TOHAND)
...
function s.filter1(c)
	return c:IsSetCard(0xa056) and c:IsAbleToRemove() and c:GetCode() ~= id -- cannot be "Leaf-shaped Behe-moth"
end
function s.cost1(e, tp, eg, ep, ev, re, r, rp, chk)
	local c = e:GetHandler()

	if chk == 0 then
		return c:IsDiscardable() and Duel.IsExistingMatchingCard(s.filter1, tp, LOCATION_DECK, 0, 1, nil)
	end
...
function s.tg2(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
	...
	if chkc then
		return chkc:IsFaceup() and chkc:IsAbleToHand()
	end
	if chk == 0 then
	...
	Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_REMOVE)
	local bc = Duel.SelectTarget(tp, Card.IsFaceup, tp, LOCATION_ONFIELD, LOCATION_ONFIELD, 1, 1, nil)
	if #bc > 0 then
...
function s.tg3(e, tp, eg, ep, ev, re, r, rp, chk, chkc)
	...
	if chkc then
		return chkc:IsLocation(LOCATION_GRAVE) and Duel.IsPlayerCanRemove(tp, chkc)
	end
	if chk == 0 then
